import"./chunk-LQ6M4NCU.js";var r=`import {Component, computed, type ElementRef, signal, viewChild} from '@angular/core';
...
const OPERATORS = new Set(['-', '+', '\xF7', '\xD7']);
...
export default class Example {
    ...
    private appendOperator(expr: string, op: string): string {
        return /[+\\-\xD7\xF7]\\s*$/.test(expr)
            ? expr.replace(/[+\\-\xD7\xF7]\\s*$/, \`\${op} \`)
            : \`\${expr} \${op} \`;
    }

    private appendDot(expr: string): string {
        const lastNumber = expr.split(/[-+*/\xD7\xF7()\\s]/).pop() ?? '';
