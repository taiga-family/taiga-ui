...
import {TUI_TABLE_OPTIONS, TuiDirection} from '../table.options';
    ...
    implements AfterViewInit, TuiTextfieldOptions, OnChanges
{
    ...
    @Output()
    public readonly directionChange = new EventEmitter<TuiDirection>();
    ...
    public updateSorterAndDirection(sorter: TuiComparator<T> | null): void {
        if (this.sorter === sorter) {
            this.updateDirection(
                this.direction === TuiDirection.Asc
                    ? TuiDirection.Desc
                    : TuiDirection.Asc,
            );
        } else {
    ...
    private updateDirection(direction: TuiDirection): void {
