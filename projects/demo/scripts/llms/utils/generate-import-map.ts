...
async function extractExportsFromFile(filePath: string): Promise<string[]> {
    ...
    for (const match of exportMatches) {
        if (match[1]) {
            ...
            // Filter out ALL_CAPS constants (tokens are handled separately)
...
function categorizeSymbol(name: string, isTypeImport: boolean): SymbolCategory {
...
async function extractImportsFromDemoPages(
    ...
    for (const [name, isTypeOnly] of symbolEntries) {
        const category = categorizeSymbol(name, isTypeOnly);
