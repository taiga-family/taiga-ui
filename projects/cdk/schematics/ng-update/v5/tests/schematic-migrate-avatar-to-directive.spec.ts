...
`;
    ...
    it('moves src value to tuiAvatar attribute on host element', async () => {
        const {template} = await runMigration({
            collection,
            component: COMPONENT,
            template: '<a tuiAvatar src="avatar.png"></a>',
        });

        expect(template).toEqual('<a tuiAvatar="avatar.png"></a>');
    });
    ...
    it('unwraps tuiFallbackSrc binding into tuiAvatar with image inside', async () => {
        const {template} = await runMigration({
            ...
            component: COMPONENT,
            template:
                '<a [tuiAvatar]="avatar$ | tuiFallbackSrc: \'@tui.user\' | async"></a>',
        });

        expect(template).toEqual('<a tuiAvatar="@tui.user"><img [src]="avatar$" /></a>');
    });
