...
export class TuiScrollService {
    ...
    public scroll$(
        ...
        if (ngDevMode) {
            console.assert(duration >= 0, 'duration cannot be negative');
            console.assert(scrollTop >= 0, 'scrollTop cannot be negative');
            console.assert(scrollLeft >= 0, 'scrollLeft cannot be negative');
        }
