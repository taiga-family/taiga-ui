...
export class TuiTableBarsHostComponent {
    ...
    public getMode(mode: TuiBrightness): TuiBrightness | null {
        return mode === 'onLight' ? 'onDark' : null;
    }

    public onCloseClick(itemToRemove: TuiTableBar): void {
        itemToRemove.close();
    }

    public getItemContext(item: TuiTableBar): TuiContext<() => void> {
        return {
            $implicit: () => item.close(),
        };
    }
