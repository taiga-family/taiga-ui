...
export class TuiTreeService<T> {
    readonly #loading = inject<T>(TUI_TREE_LOADING);
    readonly #start = inject<T>(TUI_TREE_START);
    readonly #loader = inject<TuiTreeLoader<T>>(TUI_TREE_LOADER);
    readonly #map = new Map<T, readonly T[]>([[this.#loading, []]]);
    readonly #load$ = new Subject<T>();
