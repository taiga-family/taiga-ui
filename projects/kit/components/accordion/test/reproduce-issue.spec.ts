...
import { TuiAccordion, TuiAccordionDirective } from '@taiga-ui/kit/components/accordion';
...
describe('Accordion with tuiItem issue reproduction', () => {
  ...
  it('should create child component only once when accordion opens', () => {
    ...
    // Child should be created only once
    ...
    // Should still be only one instantiation
    expect(TestChild.constructorCallCount).toBe(1);
  });
});