...
export class TuiCarouselComponent {
    ...
    public onTransitioned(transitioned: boolean): void {
    ...
    @tuiPure
    public getStyle(itemsCount: number): Partial<CSSStyleDeclaration> {
    ...
    public onSwipe(direction: TuiSwipeDirection): void {
        if (direction === 'left') {
            this.next();
