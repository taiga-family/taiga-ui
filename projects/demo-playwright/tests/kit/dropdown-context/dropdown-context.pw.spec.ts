...
test.describe('DropdownContext', () => {
    test.beforeEach(async ({page}, testInfo) => {
        ...
        await tuiGoto(page, DemoRoute.DropdownContext);
    });
    ...
    test('opens dropdown on right click', async ({page}) => {
        ...
        await expect.soft(example).toHaveScreenshot('01-dropdown-context.png');
    });
    ...
    test('closes previous dropdown after new one is opened', async ({page}) => {
        ...
        await expect.soft(example).toHaveScreenshot('02-dropdown-context.png');

        await api.focusOnBody();
        await tr.nth(2).click({button: 'right', position: {x: 1, y: 1}});
        await api.waitStableState();

        await expect.soft(example).toHaveScreenshot('03-dropdown-context.png');
    });
    ...
    test('does not close dropdown when nested dropdown is clicked', async ({page}) => {
        ...
        await expect.soft(example).toHaveScreenshot('04-dropdown-context.png');
