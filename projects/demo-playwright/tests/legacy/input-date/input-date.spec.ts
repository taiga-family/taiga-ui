...
test.describe("InputDate", () => {
        ...
        test("correct filler display for size", async ({ page }) => {
            ...
            for (const size of ["s", "m", "l"]) {
                ...
                await expect(page).toHaveScreenshot(
                    `01-01-input-date-${size}.png`,
                );

                await input.clear();

                await expect(page).toHaveScreenshot(
                    `01-02-input-date-${size}.png`,
                );
    ...
    test.describe("API", () => {
        ...
        ["s", "m", "l"].forEach((size) => {
            test(`correct filler display for size=${size}`, async ({
                page,
            }) => {
                await tuiGoto(
                    page,
                ...
                await expect(page).toHaveScreenshot(
                    `02-01-input-date-${size}.png`,
                );

                await page.keyboard.type("01.");

                await expect(page).toHaveScreenshot(
                    `02-02-input-date-${size}.png`,
                );

                await page.keyboard.type("06.1994");

                await expect(page).toHaveScreenshot(
                    `02-03-input-date-${size}.png`,
                );
            });
        });
        ...
        test("Click any day after `Until today` was selected", async ({
            page,
        }) => {
    ...
    test.describe("Mobile", () => {
        ...
        ["russian", "english", "spanish"].forEach((language) => {
            test(`mobile calendar - ${language}`, async ({ page }) => {
                ...
                await api.prepareBeforeScreenshot();
                await example
                    .locator("tui-input-date .t-icon tui-icon")
                    .click();

                await expect(page).toHaveScreenshot(
                    `08-input-date-${language}.png`,
                );
            });
        });
        ...
        test("native data picker", async ({ page }) => {
            ...
            await example.scrollIntoViewIfNeeded();
            await example
                .locator('tui-input-date input[type="date"]')
                .fill("2010-03-14");
