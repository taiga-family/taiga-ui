...
export class TuiLineDaysChartComponent implements AfterViewInit {
    ...
    public get months(): ReadonlyArray<readonly TuiPoint[]> {
        return this.value.length ? this.breakMonths(this.value) : EMPTY_ARRAY;
    }

    public get firstWidth(): number {
    ...
    public raise(index: number, {value}: TuiLineChartComponent): void {
    ...
    public getContext(index: number, {value}: TuiLineChartComponent): unknown {
        const x = value[index][0];
