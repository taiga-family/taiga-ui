describe(`Slider`, () => {
    ...
    it(`correctly displays values with float percentage progress`, () => {
        cy.tuiVisit(`components/slider/API?tuiMode=null&max=89&min=0&step=1&size=s`);
    ...
    it(`correctly sets control value on input change (using TuiSliderKeyStepsDirective)`, () => {
        cy.tuiVisit(`components/slider`);

        cy.get(`#key-steps`).tuiFindByExampleId().should(`be.visible`).as(`example`);
    ...
    it(`with \`min\` > 0`, () => {
        cy.tuiVisit(`components/slider/API?min=1&max=10&segments=9`);
    ...
    it(`with \`min\` < 0 && \`max\` > 0 `, () => {
        cy.tuiVisit(`components/slider/API?min=-5&max=5&segments=5`);
