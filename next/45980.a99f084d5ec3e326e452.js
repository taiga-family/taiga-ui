"use strict";
(self["webpackChunk_taiga_ui_components"] = self["webpackChunk_taiga_ui_components"] || []).push([[45980],{
...
;// CONCATENATED MODULE: ./node_modules/@tiptap/extension-placeholder/node_modules/prosemirror-view/dist/index.js
...
function posAtCoords(view, coords) {
  ...
  let elt = (view.root.elementFromPoint ? view.root : doc).elementFromPoint(coords.left, coords.top + 1);
  let pos;
...
class WidgetViewDesc extends (/* unused pure expression or super */ null && (ViewDesc)) {
...
class CompositionViewDesc extends (/* unused pure expression or super */ null && (ViewDesc)) {
...
class MarkViewDesc extends (/* unused pure expression or super */ null && (ViewDesc)) {
...
class NodeViewDesc extends (/* unused pure expression or super */ null && (ViewDesc)) {
  ...
  updateChildren(view, pos) {
    ...
    iterDeco(this.node, this.innerDeco, (widget, i, insideNode) => {
      if (widget.spec.marks) updater.syncToMarks(widget.spec.marks, inline, view);else if (widget.type.side >= 0 && !insideNode) updater.syncToMarks(i == this.node.childCount ? Mark.none : this.node.child(i).marks, inline, view); // If the next node is a desc matching this widget, reuse it,
...
class TextViewDesc extends (/* unused pure expression or super */ null && (NodeViewDesc)) {
...
class TrailingHackViewDesc extends (/* unused pure expression or super */ null && (ViewDesc)) {
  parseRule() {
    return {
      ignore: true
    };
...
class CustomNodeViewDesc extends (/* unused pure expression or super */ null && (NodeViewDesc)) {
...
function selectionFromDOM(view, origin = null) {
  ...
  if (selectionCollapsed(domSel)) {
    ...
    if (nearestDesc && nearestDescNode.isAtom && dist/* NodeSelection.isSelectable */.qv.isSelectable(nearestDescNode) && nearestDesc.parent && !(nearestDescNode.isInline && isOnEdge(domSel.focusNode, domSel.focusOffset, nearestDesc.dom))) {
      let pos = nearestDesc.posBefore;
      selection = new dist/* NodeSelection */.qv(head == pos ? $head : doc.resolve(pos));
    }
  } else {
...
function selectionToDOM(view, force = false) {
  ...
  if (view.cursorWrapper) {
    ...
    if (brokenSelectBetweenUneditable && !(sel instanceof dist/* TextSelection */.Bs)) {
...
function selectionBetween(view, $anchor, $head, bias) {
  return view.someProp("createSelectionBetween", f => f(view, $anchor, $head)) || dist/* TextSelection.between */.Bs.between($anchor, $head, bias);
}
...
function moveSelectionBlock(state, dir) {
  ...
  return $start && dist/* Selection.findFrom */.Y1.findFrom($start, dir);
}
...
function selectHorizontally(view, dir, mods) {
  ...
  if (sel instanceof dist/* TextSelection */.Bs) {
    if (!sel.empty || mods.indexOf("s") > -1) {
      ...
      if (next && next instanceof dist/* NodeSelection */.qv) return apply(view, next);
      return false;
      ...
      if (dist/* NodeSelection.isSelectable */.qv.isSelectable(node)) {
        return apply(view, new dist/* NodeSelection */.qv(dir < 0 ? view.state.doc.resolve($head.pos - node.nodeSize) : $head));
      } else if (webkit) {
        // Chrome and Safari will introduce extra pointless cursor
        // positions around inline uneditable nodes, so we have to
        // take over and move the cursor past them (#937)
        return apply(view, new dist/* TextSelection */.Bs(view.state.doc.resolve(dir < 0 ? nodePos : nodePos + node.nodeSize)));
      } else {
        return false;
      }
    }
  } else if (sel instanceof dist/* NodeSelection */.qv && sel.node.isInline) {
    return apply(view, new dist/* TextSelection */.Bs(dir > 0 ? sel.$to : sel.$from));
  } else {
    let next = moveSelectionBlock(view.state, dir);
...
function selectVertically(view, dir, mods) {
  let sel = view.state.selection;
  if (sel instanceof dist/* TextSelection */.Bs && !sel.empty || mods.indexOf("s") > -1) return false;
  ...
  if (!$from.parent.inlineContent) {
    ...
    let beyond = sel instanceof dist/* AllSelection */.C1 ? dist/* Selection.near */.Y1.near(side, dir) : dist/* Selection.findFrom */.Y1.findFrom(side, dir);
...
function stopNativeHorizontalDelete(view, dir) {
  if (!(view.state.selection instanceof dist/* TextSelection */.Bs)) return true;
  let {
    $head,
    $anchor,
    empty
...
function serializeForClipboard(view, slice) {
  ...
  let serializer = view.someProp("clipboardSerializer") || prosemirror_model_dist/* DOMSerializer.fromSchema */.PW.fromSchema(view.state.schema);
  let doc = detachedDoc(),
...
function parseFromClipboard(view, text, html, plainText, $context) {
  ...
  if (asText) {
    ...
    if (inCode) return text ? new prosemirror_model_dist/* Slice */.p2(prosemirror_model_dist/* Fragment.from */.HY.from(view.state.schema.text(text.replace(/\r\n?/g, "\n"))), 0, 0) : prosemirror_model_dist/* Slice.empty */.p2.empty;
    ...
    if (parsed) {
      ...
      let {
        schema
          ...
          serializer = prosemirror_model_dist/* DOMSerializer.fromSchema */.PW.fromSchema(schema);
  ...
  if (!slice) {
    let parser = view.someProp("clipboardParser") || view.someProp("domParser") || prosemirror_model_dist/* DOMParser.fromSchema */.aw.fromSchema(view.state.schema);
  ...
  if (sliceData) {
    ...
    slice = prosemirror_model_dist/* Slice.maxOpen */.p2.maxOpen(normalizeSiblings(slice.content, $context), true);
...
function normalizeSiblings(fragment, $context) {
  ...
  for (let d = $context.depth; d >= 0; d--) {
    ...
    if (result) return prosemirror_model_dist/* Fragment.from */.HY.from(result);
  }
...
function withWrappers(node, wrap, from = 0) {
  for (let i = wrap.length - 1; i >= from; i--) node = wrap[i].create(null, prosemirror_model_dist/* Fragment.from */.HY.from(node));
...
function addToSibling(wrap, lastWrap, node, sibling, depth) {
  if (depth < wrap.length && depth < lastWrap.length && wrap[depth] == lastWrap[depth]) {
    ...
    if (match.matchType(depth == wrap.length - 1 ? node.type : wrap[depth + 1])) return sibling.copy(sibling.content.append(prosemirror_model_dist/* Fragment.from */.HY.from(withWrappers(node, wrap, depth + 1))));
  }
}
...
function closeRight(node, depth) {
  ...
  let fill = node.contentMatchAt(node.childCount).fillBefore(prosemirror_model_dist/* Fragment.empty */.HY.empty, true);
...
function closeRange(fragment, side, from, to, depth, openEnd) {
  ...
  if (depth >= from) inner = side < 0 ? node.contentMatchAt(0).fillBefore(inner, fragment.childCount > 1 || openEnd <= depth).append(inner) : inner.append(node.contentMatchAt(node.childCount).fillBefore(prosemirror_model_dist/* Fragment.empty */.HY.empty, true));
  return fragment.replaceChild(side < 0 ? 0 : fragment.childCount - 1, node.copy(inner));
}

function closeSlice(slice, openStart, openEnd) {
  if (openStart < slice.openStart) slice = new prosemirror_model_dist/* Slice */.p2(closeRange(slice.content, -1, openStart, slice.openStart, 0, slice.openEnd), openStart, slice.openEnd);
  if (openEnd < slice.openEnd) slice = new prosemirror_model_dist/* Slice */.p2(closeRange(slice.content, 1, openEnd, slice.openEnd, 0, 0), slice.openStart, openEnd);
  return slice;
...
function addContext(slice, context) {
  ...
  for (let i = array.length - 2; i >= 0; i -= 2) {
    ...
    content = prosemirror_model_dist/* Fragment.from */.HY.from(type.create(array[i + 1], content));
    openStart++;
    openEnd++;
  }

  return new prosemirror_model_dist/* Slice */.p2(content, openStart, openEnd);
...
editHandlers.keypress = (view, _event) => {
  ...
  if (!(sel instanceof dist/* TextSelection */.Bs) || !sel.$from.sameParent(sel.$to)) {
...
function selectClickedLeaf(view, inside) {
  ...
  if (node && node.isAtom && dist/* NodeSelection.isSelectable */.qv.isSelectable(node)) {
    updateSelection(view, new dist/* NodeSelection */.qv($pos), "pointer");
    return true;
  }
...
function selectClickedNode(view, inside) {
  ...
  let sel = view.state.selection,
      selectedNode,
      selectAt;
  if (sel instanceof dist/* NodeSelection */.qv) selectedNode = sel.node;
  let $pos = view.state.doc.resolve(inside);
  ...
  if (selectAt != null) {
    updateSelection(view, dist/* NodeSelection.create */.qv.create(view.state.doc, selectAt), "pointer");
    return true;
  } else {
    return false;
  }
...
function defaultTripleClick(view, inside, event) {
  ...
  if (inside == -1) {
    if (doc.inlineContent) {
      updateSelection(view, dist/* TextSelection.create */.Bs.create(doc, 0, doc.content.size), "pointer");
      return true;
    }
  ...
  for (let i = $pos.depth + 1; i > 0; i--) {
    ...
    if (node.inlineContent) updateSelection(view, dist/* TextSelection.create */.Bs.create(doc, nodePos + 1, nodePos + 1 + node.content.size), "pointer");else if (dist/* NodeSelection.isSelectable */.qv.isSelectable(node)) updateSelection(view, dist/* NodeSelection.create */.qv.create(doc, nodePos), "pointer");else continue;
    return true;
  }
}
...
class MouseDown {
  constructor(view, pos, event, flushed) {
    ...
    if (event.button == 0 && targetNode.type.spec.draggable && targetNode.type.spec.selectable !== false || selection instanceof dist/* NodeSelection */.qv && selection.from <= targetPos && selection.to > targetPos) this.mightDrag = {
      node: targetNode,
      pos: targetPos,
  ...
  up(event) {
    ...
    // works around that.
    chrome && !(this.view.state.selection instanceof dist/* TextSelection */.Bs) && Math.min(Math.abs(pos.pos - this.view.state.selection.from), Math.abs(pos.pos - this.view.state.selection.to)) <= 2)) {
      updateSelection(this.view, dist/* Selection.near */.Y1.near(this.view.state.doc.resolve(pos.pos)), "pointer");
      event.preventDefault();
    } else {
      setSelectionOrigin(this.view, "pointer");
    }
...
function doPaste(view, text, html, event) {
  ...
  if (view.someProp("handlePaste", f => f(view, event, slice || prosemirror_model_dist/* Slice.empty */.p2.empty))) return true;
...
handlers.dragstart = (view, _event) => {
  ...
  if (pos && pos.pos >= sel.from && pos.pos <= (sel instanceof dist/* NodeSelection */.qv ? sel.to - 1 : sel.to)) ;else if (mouseDown && mouseDown.mightDrag) {
    view.dispatch(view.state.tr.setSelection(dist/* NodeSelection.create */.qv.create(view.state.doc, mouseDown.mightDrag.pos)));
  } else if (event.target && event.target.nodeType == 1) {
    let desc = view.docView.nearestDesc(event.target, true);
    if (desc && desc.node.type.spec.draggable && desc != view.docView) view.dispatch(view.state.tr.setSelection(dist/* NodeSelection.create */.qv.create(view.state.doc, desc.posBefore)));
  }
...
editHandlers.drop = (view, _event) => {
  ...
  if (view.someProp("handleDrop", f => f(view, event, slice || prosemirror_model_dist/* Slice.empty */.p2.empty, move))) {
    event.preventDefault();
    return;
  }
  ...
  if (!slice) return;
  event.preventDefault();
  let insertPos = slice ? (0,prosemirror_transform_dist/* dropPoint */.nj)(view.state.doc, $mouse.pos, slice) : $mouse.pos;
  ...
  if (isNode && dist/* NodeSelection.isSelectable */.qv.isSelectable(slice.content.firstChild) && $pos.nodeAfter && $pos.nodeAfter.sameMarkup(slice.content.firstChild)) {
    tr.setSelection(new dist/* NodeSelection */.qv($pos));
  } else {
  ...
  */
  ...
  updateStateInner(state, reconfigured) {
    ...
    if (scroll == "reset") {
      ...
      if (this.someProp("handleScrollToSelection", f => f(this))) ;else if (state.selection instanceof NodeSelection) {
...
;// CONCATENATED MODULE: ./node_modules/@tiptap/extension-placeholder/dist/tiptap-extension-placeholder.esm.js



const Placeholder = tiptap_core_esm.Extension.create({
  name: 'placeholder',
