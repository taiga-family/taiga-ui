"use strict";
(self["webpackChunk_taiga_ui_components"] = self["webpackChunk_taiga_ui_components"] || []).push([[84101],{
...
;// CONCATENATED MODULE: ./node_modules/@tiptap/extension-placeholder/node_modules/prosemirror-state/dist/index.js
...
class dist_Selection {
  ...
  */
  ...
  static findFrom($pos, dir, textOnly = false) {
    let inner = $pos.parent.inlineContent ? new dist_TextSelection($pos) : findSelectionIn($pos.node(0), $pos.parent, $pos.pos, $pos.index(), dir, textOnly);
...
class dist_TextSelection extends dist_Selection {
  ...
  */
  ...
  map(doc, mapping) {
    ...
    if (!$head.parent.inlineContent) return dist_Selection.near($head);
  ...
  */
  ...
  static between($anchor, $head, bias) {
    ...
    if (!$head.parent.inlineContent) {
      ...
      if (found) $head = found.$head;else return dist_Selection.near($head, bias);
    }
...
class dist_NodeSelection extends dist_Selection {
  ...
  */
  ...
  map(doc, mapping) {
    ...
    if (deleted) return dist_Selection.near($pos);
...
class NodeBookmark {
  ...
  resolve(doc) {
    let $pos = doc.resolve(this.anchor),
        node = $pos.nodeAfter;
    if (node && dist_NodeSelection.isSelectable(node)) return new dist_NodeSelection($pos);
  ...
  */
  ...
  insertText(text, from, to) {
    ...
    if (from == null) {
      ...
      if (!this.selection.empty) this.setSelection(dist_Selection.near(this.selection.$to));
      return this;
    }
  }
  /**
...
const baseFields = [new FieldDesc("doc", {
  ...
  init(config, instance) {
    return config.selection || dist_Selection.atStart(instance.doc);
  },
...
function selectionFromDOM(view, origin = null) {
  ...
  if (selectionCollapsed(domSel)) {
    ...
    if (nearestDesc && nearestDescNode.isAtom && dist_NodeSelection.isSelectable(nearestDescNode) && nearestDesc.parent && !(nearestDescNode.isInline && isOnEdge(domSel.focusNode, domSel.focusOffset, nearestDesc.dom))) {
...
function selectionBetween(view, $anchor, $head, bias) {
  return view.someProp("createSelectionBetween", f => f(view, $anchor, $head)) || dist_TextSelection.between($anchor, $head, bias);
}
...
function selectHorizontally(view, dir, mods) {
  ...
  if (sel instanceof dist_TextSelection) {
    if (!sel.empty || mods.indexOf("s") > -1) {
      ...
      if (next && next instanceof dist_NodeSelection) return apply(view, next);
      return false;
...
function selectVertically(view, dir, mods) {
  let sel = view.state.selection;
  if (sel instanceof dist_TextSelection && !sel.empty || mods.indexOf("s") > -1) return false;
...
function stopNativeHorizontalDelete(view, dir) {
  if (!(view.state.selection instanceof dist_TextSelection)) return true;
  let {
    $head,
    $anchor,
    empty
...
function serializeForClipboard(view, slice) {
  ...
  let serializer = view.someProp("clipboardSerializer") || dist/* DOMSerializer.fromSchema */.PW.fromSchema(view.state.schema);
  let doc = detachedDoc(),
...
function parseFromClipboard(view, text, html, plainText, $context) {
  ...
  if (asText) {
    ...
    if (inCode) return text ? new dist/* Slice */.p2(dist/* Fragment.from */.HY.from(view.state.schema.text(text.replace(/\r\n?/g, "\n"))), 0, 0) : dist/* Slice.empty */.p2.empty;
    ...
    if (parsed) {
      ...
      let {
        schema
          ...
          serializer = dist/* DOMSerializer.fromSchema */.PW.fromSchema(schema);
  ...
  if (!slice) {
    let parser = view.someProp("clipboardParser") || view.someProp("domParser") || dist/* DOMParser.fromSchema */.aw.fromSchema(view.state.schema);
  ...
  if (sliceData) {
    ...
    slice = dist/* Slice.maxOpen */.p2.maxOpen(normalizeSiblings(slice.content, $context), true);
...
function normalizeSiblings(fragment, $context) {
  ...
  for (let d = $context.depth; d >= 0; d--) {
    ...
    if (result) return dist/* Fragment.from */.HY.from(result);
  }
...
function withWrappers(node, wrap, from = 0) {
  for (let i = wrap.length - 1; i >= from; i--) node = wrap[i].create(null, dist/* Fragment.from */.HY.from(node));
...
function addToSibling(wrap, lastWrap, node, sibling, depth) {
  if (depth < wrap.length && depth < lastWrap.length && wrap[depth] == lastWrap[depth]) {
    ...
    if (match.matchType(depth == wrap.length - 1 ? node.type : wrap[depth + 1])) return sibling.copy(sibling.content.append(dist/* Fragment.from */.HY.from(withWrappers(node, wrap, depth + 1))));
  }
}
...
function closeRight(node, depth) {
  ...
  let fill = node.contentMatchAt(node.childCount).fillBefore(dist/* Fragment.empty */.HY.empty, true);
...
function closeRange(fragment, side, from, to, depth, openEnd) {
  ...
  if (depth >= from) inner = side < 0 ? node.contentMatchAt(0).fillBefore(inner, fragment.childCount > 1 || openEnd <= depth).append(inner) : inner.append(node.contentMatchAt(node.childCount).fillBefore(dist/* Fragment.empty */.HY.empty, true));
  return fragment.replaceChild(side < 0 ? 0 : fragment.childCount - 1, node.copy(inner));
}

function closeSlice(slice, openStart, openEnd) {
  if (openStart < slice.openStart) slice = new dist/* Slice */.p2(closeRange(slice.content, -1, openStart, slice.openStart, 0, slice.openEnd), openStart, slice.openEnd);
  if (openEnd < slice.openEnd) slice = new dist/* Slice */.p2(closeRange(slice.content, 1, openEnd, slice.openEnd, 0, 0), slice.openStart, openEnd);
  return slice;
...
function addContext(slice, context) {
  ...
  for (let i = array.length - 2; i >= 0; i -= 2) {
    ...
    content = dist/* Fragment.from */.HY.from(type.create(array[i + 1], content));
    openStart++;
    openEnd++;
  }

  return new dist/* Slice */.p2(content, openStart, openEnd);
...
function selectClickedNode(view, inside) {
  ...
  let sel = view.state.selection,
      selectedNode,
      selectAt;
  if (sel instanceof dist_NodeSelection) selectedNode = sel.node;
  let $pos = view.state.doc.resolve(inside);
...
function defaultTripleClick(view, inside, event) {
  ...
  for (let i = $pos.depth + 1; i > 0; i--) {
    ...
    if (node.inlineContent) updateSelection(view, dist_TextSelection.create(doc, nodePos + 1, nodePos + 1 + node.content.size), "pointer");else if (dist_NodeSelection.isSelectable(node)) updateSelection(view, dist_NodeSelection.create(doc, nodePos), "pointer");else continue;
    return true;
  }
}
...
class MouseDown {
  constructor(view, pos, event, flushed) {
    ...
    if (event.button == 0 && targetNode.type.spec.draggable && targetNode.type.spec.selectable !== false || selection instanceof dist_NodeSelection && selection.from <= targetPos && selection.to > targetPos) this.mightDrag = {
      node: targetNode,
      pos: targetPos,
  ...
  up(event) {
    ...
    // works around that.
    chrome && !(this.view.state.selection instanceof dist_TextSelection) && Math.min(Math.abs(pos.pos - this.view.state.selection.from), Math.abs(pos.pos - this.view.state.selection.to)) <= 2)) {
...
function doPaste(view, text, html, event) {
  ...
  if (view.someProp("handlePaste", f => f(view, event, slice || dist/* Slice.empty */.p2.empty))) return true;
...
handlers.dragstart = (view, _event) => {
  ...
  if (pos && pos.pos >= sel.from && pos.pos <= (sel instanceof dist_NodeSelection ? sel.to - 1 : sel.to)) ;else if (mouseDown && mouseDown.mightDrag) {
    ...
    if (desc && desc.node.type.spec.draggable && desc != view.docView) view.dispatch(view.state.tr.setSelection(dist_NodeSelection.create(view.state.doc, desc.posBefore)));
  }
...
editHandlers.drop = (view, _event) => {
  ...
  if (view.someProp("handleDrop", f => f(view, event, slice || dist/* Slice.empty */.p2.empty, move))) {
    event.preventDefault();
    return;
  }
  ...
  if (isNode && dist_NodeSelection.isSelectable(slice.content.firstChild) && $pos.nodeAfter && $pos.nodeAfter.sameMarkup(slice.content.firstChild)) {
